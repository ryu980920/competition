# 개발 로그 아카이브 — FinFET 전환 이전 (2026-07-28 ~ 2026-08-02)

> 이 문서는 [`devlog.md`](devlog.md)에서 분리한 **원본 날짜별 로그**다. 대상 기간은 GAA-TFET로 시작해 GAA 초협소 시트간격 Vt-Implant, DRAM BCAT(Rfillet), DRAM BCAT(DBCAT×접합 도핑)를 거쳐 2026-08-04에 최종적으로 FinFET + Embedded SiGe S/D로 전환되기 전까지다 — 지금은 전부 폐기된 경로다.
>
> 왜 각 후보가 기각됐는지 압축된 서술은 [`topic-selection-history.md`](topic-selection-history.md)에 정리돼 있다. 이 문서는 그날그날 실제로 무엇을 했는지 원문 그대로를 보존하기 위한 것이다. 내용은 분리 당시 `devlog.md` 원문 그대로이며 새로 수정하지 않는다.
