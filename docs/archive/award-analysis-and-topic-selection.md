...
> **[읽기 전 안내] 이 문서의 위치**
>
> 이 문서는 프로젝트 초기(2026-07-28)에 작성된 **수상작 조사 자료**다. 2장(수상작 목록)과 3장(패턴 분석)은 당시 DRAM BCAT를 후보 주제로 선택하는 근거가 되었다. 이후 최종 주제는 FinFET Embedded SiGe S/D로 전환되었으며, 현재 결과는 [최종 결과 요약](../reports/project-summary.md)을 따른다. 아래는 당시 판단의 기록이다.
>
> - DRAM 셀 트랜지스터가 SIF 2023·KCS 2025에서 이미 두 차례 수상한 카테고리라는 점
> - 파라미터 최적화 유형(BCAT Multiple-energy ion implantation)도 최고상(사업단장상)을 받을 수 있다는 점
>
> 다만 **5~6장의 결론(GAA 시트별 차등 Halo Doping 추천)은 폐기되었다.** 이후 GAA 계열 전체가 문헌 중복·양산성 문제로 기각되고 DRAM BCAT로 전환되었기 때문이다.
>
> - 현재 주제: [`README.md`](../../README.md)
> - 전환 과정 전체: [`docs/topic-selection-history.md`](../topic-selection-history.md) — 검토·기각한 19개 후보 아카이브
>
> 5~6장은 "왜 파라미터 단순 조합만으로는 독창성을 인정받기 어려운가"에 대한 당시의 판단 근거로서 그대로 보존한다.
